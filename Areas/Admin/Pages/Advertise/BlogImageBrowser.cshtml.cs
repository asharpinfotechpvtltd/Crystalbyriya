...
{
    public class BlogImageBrowserModel : PageModel
    {
        public BlogImageBrowserModel(IConfiguration config, IWebHostEnvironment env)
        {
            _config = config;
            _env = env;
        }
        ...
        /// </summary>
        private string ImageFolderRoot => _config["FliersRoot"] != null ? $"/{_config["FliersRoot"].Trim('/')}/" : "";
        ...
        public async Task OnPostUploadAsync()
        {
            if (IsImage(UploadedImageFile.FileName))
            {
                ...
                //System.IO.File.WriteAllBytes("https//www.astaberry.com/Fliers/" + FileImageFolder + filename, image);
